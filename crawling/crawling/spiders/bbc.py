...
class BBCSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in links:
            ...
            elif "/news/" in link and not any(
                x in link for x in ["#", "mailto:", "live", "comments", "in-pictures"]
            ):
    ...
    def is_article_page(self, url):
        ...
        return (
            re.search(r"/news/.*-\d+$", url) is not None
            or re.search(r"/news/articles/[a-z0-9]+", url) is not None
        )
    ...
    def parse_article(self, response):
        ...
        if not title:
            self.logger.warning(
                f"Could not extract title from {response.url}, might not be an article"
            )
            return None
        ...
        authors = response.css(
            'article [data-component="byline-block"] *::text, article header [data-component="byline"] *::text'
        ).getall()
