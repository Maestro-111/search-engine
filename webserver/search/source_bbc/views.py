...
def search_bbc(request):
    ...
    if query:
        ...
        if raw_results is None:
            ...
            raw_results = es.query_specified_fields(search_body=search_body, index="bbc")
...
def crawl_bbc(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Log the request
            logger.info(f"Starting bbc crawl {starting_url} with depth {crawl_depth} and max_pages {max_pages}. "
            ...
            messages.success(request, "BBC Crawl/Index job has been submitted successfully.")
